...
# ============================================================================
# Website Crawling API Endpoints
# ============================================================================

from schemas import (
    WebsiteCrawlCreate, WebsiteCrawlResponse, WebsiteCrawlStatus,
    WebsiteChatRequest, WebsiteChatResponse, SourceCitation
)
from models import WebsiteCrawl, TenantCollection, Agent
from rag_model.rag_website import (
    create_or_get_opensearch_collection,
    create_or_get_knowledge_base,
...
@app.post("/tenants/{tenant_id}/websites/crawl", status_code=200)
async def initiate_website_crawl(
    tenant_id: int,  # This is actually agent_id
    crawl_request: WebsiteCrawlCreate,
    background_tasks: BackgroundTasks,
    db: Session = Depends(get_db)
):
    """
    Initiate website crawling. Returns immediately with 200 OK.
    
    The crawl happens in background. Use GET /tenants/{agent_id}/websites/status to check progress.
    """
    try:
        ...
        if not agent:
            agent = Agent(id=agent_id, agent_name=f"Agent {agent_id}")
            db.add(agent)
            db.commit()
            db.refresh(agent)
        
        # Create website_crawls record
        ...
        return {
            "status": "success",
            "message": f"Crawl initiated for {crawl_request.url}. Use GET /tenants/{agent_id}/websites/status to check progress.",
            "agent_id": agent_id
        }
...
@app.get("/tenants/{tenant_id}/websites/status")
async def get_agent_crawl_status(
    tenant_id: int,  # This is actually agent_id
    db: Session = Depends(get_db)
):
    """
    Get current status of all website crawls for an agent.
    
    Returns list of all crawls with their current status.
    """
    try:
        agent_id = tenant_id
        
        # Get all website crawls for this agent, ordered by most recent first
        crawls = db.query(WebsiteCrawl).filter(
            WebsiteCrawl.agent_id == agent_id
        ).order_by(WebsiteCrawl.created_at.desc()).all()
        
        if not crawls:
            return {
                "agent_id": agent_id,
                "crawls": [],
                "message": "No crawls found for this agent"
            }
...
def process_website_crawl_background(
    ...
    try:
        ...
        try:
            ...
            error_msg = f"Failed to create OpenSearch collection: {str(e)}"
            print(f"❌ {error_msg}")
            website_crawl.status = "FAILED"
            website_crawl.error_message = error_msg
            db.commit()
            return
        ...
        try:
            ...
            error_msg = f"Failed to start crawl: {str(e)}"
            print(f"❌ {error_msg}")
            website_crawl.status = "FAILED"
            website_crawl.error_message = error_msg
            db.commit()
        ...
        print(f"❌ Background: Unexpected error processing crawl: {str(e)}")
        import traceback
        traceback.print_exc()
        ...
        try:
            ...
            if website_crawl:
                website_crawl.status = "FAILED"
                website_crawl.error_message = f"Unexpected error: {str(e)}"
                db.commit()
...
async def get_crawl_status(
    ...
    """
    Get the current status of a website crawling operation.
    
    Note: tenant_id in the URL is actually used as agent_id internally.
    
    Returns cached status for COMPLETE/FAILED, queries AWS for STARTING/IN_PROGRESS.
    """
    try:
        agent_id = tenant_id
        
        # Query database for website crawl using agent_id
        website_crawl = db.query(WebsiteCrawl).filter(
            WebsiteCrawl.agent_id == agent_id,
            WebsiteCrawl.website_id == website_id
        ).first()
        
        if not website_crawl:
            raise HTTPException(
                status_code=404,
                detail=f"Website crawl not found for agent {agent_id} and website {website_id}"
            )
        
        # If status is COMPLETE or FAILED, return cached data
        if website_crawl.status in ["COMPLETE", "FAILED"]:
            return WebsiteCrawlStatus(
                website_id=website_crawl.website_id,
                website_url=website_crawl.website_url,
                knowledge_base_id=website_crawl.knowledge_base_id,
                status=website_crawl.status,
                pages_crawled=website_crawl.pages_crawled,
                max_pages=website_crawl.max_pages,
                start_time=website_crawl.created_at,
                completion_time=website_crawl.completed_at,
                error_message=website_crawl.error_message
            )
        
        # For STARTING or IN_PROGRESS, query AWS
        if website_crawl.status in ["STARTING", "IN_PROGRESS", "pending"]:
            status_result = check_ingestion_status(
                knowledge_base_id=website_crawl.knowledge_base_id,
                data_source_id=website_crawl.data_source_id,
                ingestion_job_id=website_crawl.ingestion_job_id,
                db=db
            )
            
            # Update database with latest status
            website_crawl.status = status_result['status']
            website_crawl.pages_crawled = status_result['pages_crawled']
            
            if status_result['status'] in ['COMPLETE', 'FAILED']:
                website_crawl.completed_at = datetime.utcnow()
                
            if status_result['status'] == 'FAILED' and status_result['failure_reasons']:
                website_crawl.error_message = '; '.join(status_result['failure_reasons'])
            ...
            return WebsiteCrawlStatus(
                ...
                pages_crawled=status_result['pages_crawled'],
                max_pages=website_crawl.max_pages,
                start_time=status_result['start_time'],
                completion_time=status_result['completion_time'],
                error_message=website_crawl.error_message
            )
        ...
        raise HTTPException(status_code=500, detail=f"Error checking crawl status: {str(e)}")


@app.post("/tenants/{tenant_id}/websites/chat", response_model=WebsiteChatResponse)
async def chat_with_website(
    tenant_id: int,  # This is actually agent_id
    chat_request: WebsiteChatRequest,
    db: Session = Depends(get_db)
):
    """
    Ask questions about a specific website using its knowledge base.
    
    Note: tenant_id in the URL is actually used as agent_id internally.
    
    The website must be crawled and indexed (status = COMPLETE) before querying.
    """
    try:
        agent_id = tenant_id
        
        # Query database for the most recent COMPLETE website crawl using agent_id
        website_crawl = db.query(WebsiteCrawl).filter(
            WebsiteCrawl.agent_id == agent_id,
            WebsiteCrawl.website_url == chat_request.website_url,
            WebsiteCrawl.status == "COMPLETE"
        ).order_by(WebsiteCrawl.created_at.desc()).first()
        
        if not website_crawl:
            raise HTTPException(
                status_code=404,
                detail=f"Website {chat_request.website_url} has not been crawled yet. Please crawl it first using POST /tenants/{agent_id}/websites/crawl"
            )
        
        # Check status
        if website_crawl.status == "STARTING":
            raise HTTPException(
                status_code=400,
                detail="Crawling is starting. Please wait and try again in a few moments."
            )
        
        if website_crawl.status == "IN_PROGRESS":
            raise HTTPException(
                status_code=400,
                detail=f"Crawling is in progress ({website_crawl.pages_crawled}/{website_crawl.max_pages} pages). Please wait until it completes."
            )
        
        if website_crawl.status == "FAILED":
            raise HTTPException(
                status_code=400,
                detail=f"Crawling failed: {website_crawl.error_message}"
            )
        
        if website_crawl.status == "pending":
            raise HTTPException(
                status_code=400,
                detail="Website crawl has not started yet. Please check the crawl status."
            )
        
        if website_crawl.status != "COMPLETE":
            raise HTTPException(
                status_code=400,
                detail=f"Website is not ready for querying. Current status: {website_crawl.status}"
            )
        
        # Query the knowledge base
        print(f"\n{'='*60}")
        print(f"💬 Querying website knowledge base")
        print(f"   Tenant: {tenant_id}")
        print(f"   Website: {chat_request.website_url}")
        print(f"   Question: {chat_request.question}")
        print(f"{'='*60}\n")
        
        result = query_website_knowledge_base(
            knowledge_base_id=website_crawl.knowledge_base_id,
            question=chat_request.question,
            num_results=12
        )
        
        # Convert citations to SourceCitation objects
        source_citations = [
            SourceCitation(
                url=citation['url'],
        ...
        print(f"   Citations: {len(source_citations)}")
        print(f"{'='*60}\n")
        ...
        print(f"❌ Error querying website: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Error querying website: {str(e)}")
